...
document.querySelectorAll('.gallery-card').forEach(card => {
  card.addEventListener('click', () => { lightboxImg.src = card.dataset.img; lightbox.classList.add('open'); });
});
...
function starsFromRating(rating) { return '⭐'.repeat(Number(rating) || 5); }
...
async function loadTestimonials() {
  ...
  try {
    ...
    if (!testimonials.length) {
      testimonialList.innerHTML = `<div class="empty-testimonial"><span>🌿</span><p>Aún no hay testimonios publicados. Sé la primera persona en compartir tu experiencia.</p></div>`;
      return;
    }
    ...
    testimonialList.innerHTML = `<div class="empty-testimonial"><span>🌿</span><p>No se pudieron cargar los testimonios ahora mismo.</p></div>`;
  }
}
async function submitTestimonial(event) {
  ...
  if (!nombre || !experiencia) { testimonialStatus.textContent = 'Completa tu nombre y tu experiencia.'; return; }
  testimonialBtn.disabled = true; testimonialBtn.textContent = 'Enviando...'; testimonialStatus.textContent = 'Guardando tu testimonio...';
  try {
    const response = await fetch(TESTIMONIOS_ENDPOINT, {
      method: 'POST',
      headers: { apikey: SUPABASE_KEY, Authorization: `Bearer ${SUPABASE_KEY}`, 'Content-Type': 'application/json', Prefer: 'return=representation' },
      body: JSON.stringify({ Nombre: nombre, 'Calificación': calificacion, Experiencia: experiencia, Fecha: fecha || new Date().toLocaleDateString('es-DO'), Aprobado: true })
    });
    ...
    testimonialBtn.disabled = false; testimonialBtn.textContent = 'Enviar testimonio';
  }
}
if (testimonialForm) testimonialForm.addEventListener('submit', submitTestimonial);
loadTestimonials();
